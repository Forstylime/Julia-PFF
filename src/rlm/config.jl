...
@inline function history_value(history::RLMPiecewiseLinearHistory, time::Real)
    t = Float64(time)
    t_min, t_max = first(history.times), last(history.times)
    endpoint_tolerance = 1e-7
    t_min - endpoint_tolerance <= t <= t_max + endpoint_tolerance || throw(ArgumentError(
        "time $t lies outside history interval [$(t_min - endpoint_tolerance), $(t_max + endpoint_tolerance)]",
    ))
    # A time generated by `step * dt` can differ from a history endpoint by a
    # few ulps. Clamp such values before searching so endpoint interpolation is
    # evaluated exactly at the endpoint.
