# src/solvers/sav_quasistatic.jl
...
"""
    solve_sav_quasistatic(setup, mat, nat; T_final=0.01, enforce_irreversibility=true)

执行 SAV 完全线性化交替求解 (准静态版本)。
"""
function solve_sav_quasistatic(
    ...
    # VTK 输出目录
    outdir = enforce_irreversibility ? "data/sims/sav_qs" : "data/sims/sav2_qs"
    mkpath(outdir)

    t_start = time()

    # --- 6. 组装并因子分解恒定的 SAV 左端矩阵 ---
    assemble_SAV_QS_LHS!(K_d, K_u, dh_u, dh_d, cv_u, cv_d, mat, nat)
    ...
    println("矩阵分解完成 ($(typeof(F_Kd).name.name), $(typeof(F_Ku).name.name))，开始准静态 SAV 格式求解，总步数: $n_steps")
    ...
    for step in 1:n_steps
        ...
        assemble_SAV_QS_RHS!(fd1, fd2, fu1, fu2, dh_u, dh_d,
            u_n, d_n, ξ_n, E_nl_n, cv_u, cv_d, mat, nat)
        ...
        # 准静态不需要更新速度场 v_np1，填入 0 以保持状态数据结构完整
        v_np1 = zeros(ndofs_u)
