...
"""
    RLMParams{T<:AbstractFloat}

存储 RLM Mode I Amor 算法特有的数值参数。

# 成员变量
- `Δt::T`: 物理时间步长
- `M::T`: 相场动力学演化的常数 (即 1/粘性系数)
- `ϵ::T`: Amor 光滑化正则参数，通常很小如 1e-5
- `θ::T`: RLM 能量松弛控制参数
"""
Base.@kwdef struct RLMParams{T<:AbstractFloat}
    Δt::T
    M::T
    ϵ::T
    θ::T

    function RLMParams(Δt::T, M::T, ϵ::T, θ::T) where {T<:AbstractFloat}
        @assert Δt > 0 "时间步长 Δt 必须大于 0"
        @assert M > 0 "迁移率 M 必须大于 0"
        @assert ϵ >= 0 "光滑化参数 ϵ 必须非负"
        @assert θ > 0 "松弛参数 θ 必须大于 0"
        return new{T}(Δt, M, ϵ, θ)
    end
end

"""
    RLMState{T<:AbstractFloat}

存储 RLM 算法所需的状态历史和中间仿射变量。

- `u::Vector{T}`, `u_n::Vector{T}`, `u_nm1::Vector{T}`: 位移场在当前步、前一步、前两步的值
- `d::Vector{T}`, `d_n::Vector{T}`, `d_nm1::Vector{T}`: 相场在当前步、前一步、前两步的值
- `u1::Vector{T}`, `u2::Vector{T}`: 位移场仿射双分支
- `d1::Vector{T}`, `d2::Vector{T}`: 相场仿射双分支
- `η_n::T`, `η_nm1::T`: 标量乘子的历史记录
"""
mutable struct RLMState{T<:AbstractFloat}
    u::Vector{T}
    u_n::Vector{T}
    u_nm1::Vector{T}
    d::Vector{T}
    ...
    function RLMState{T}(ndofs_u::Int, ndofs_d::Int) where {T<:AbstractFloat}
        return new{T}(
            zeros(T, ndofs_u), zeros(T, ndofs_u), zeros(T, ndofs_u),
            zeros(T, ndofs_d), zeros(T, ndofs_d), zeros(T, ndofs_d),
            zeros(T, ndofs_u), zeros(T, ndofs_u),
            zeros(T, ndofs_d), zeros(T, ndofs_d),
            one(T), one(T)
        )
    end
end
...
"""
    update_rlm_states!(state::RLMState, η_new::T)

在一时间步结束后更新 RLM 状态，用于 BDF2 的下一步外推。
"""
function update_rlm_states!(state::RLMState{T}, η_new::T) where {T}
    copyto!(state.u_nm1, state.u_n)
    copyto!(state.u_n, state.u)
    copyto!(state.d_nm1, state.d_n)
    copyto!(state.d_n, state.d)
    
    state.η_nm1 = state.η_n
    state.η_n = η_new
    return nothing
end