...
# 快速示例：从项目根目录运行
#   julia --project=. scripts/run_sav.jl
...
disp, force, psi_energy, gf_energy = solve_sav_quasistatic(setup, mat, nat;
    T_final=1,
    enforce_irreversibility=true,
) # 实际最终位移为 T_final * setup.final_displacement
...
save("data/plots/load_displacement_sav_qs.png", fig_load)
...
save("data/plots/energy_evolution_sav_qs.png", fig_energy)

println("载荷-位移曲线已保存至 data/plots/load_displacement_sav_qs.png。")
println("能量演变曲线已保存至 data/plots/energy_evolution_sav_qs.png。")

# 保存数据以供后续分析
using JLD2
@save "data/jld2/sav_results_qs.jld2" disp force psi_energy gf_energy
println("SAV 仿真数据已保存至 data/jld2/sav_results_qs.jld2。")